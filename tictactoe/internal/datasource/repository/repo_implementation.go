...
func (r *GameRepositoryImpl) GetFinishedGamesByID(ctx context.Context, userID uuid.UUID) ([]*model.FinishedGamesInfo, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	...
	gameModels, err := r.storage.GetFinishedGames(userID.String())
	...
	for _, m := range gameModels {
		...
		games = append(games, &model.FinishedGamesInfo{GameID: gID, State: gState, Opponent: gOpp, CreatedAt: created})
	}

	return games, nil
}

func (r *GameRepositoryImpl) GetLeaderboard(ctx context.Context, limit int) ([]*model.PlayerWinrateInfo, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	...
	gameModels, err := r.storage.GetLeaderboard(limit)
	...
	for _, m := range gameModels {
		...
		players = append(players, &model.PlayerWinrateInfo{ID: gID, Rating: gRating})
	}
