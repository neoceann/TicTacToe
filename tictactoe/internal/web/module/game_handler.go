...
func (h *GameHandler) JoinGame(w http.ResponseWriter, r *http.Request){
	...
	currentUserId, _ := uuid.Parse(userIDAsString)
...
func (h *GameHandler) GetPublicUserInfoByToken(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		mapper.WriteJSON(w, http.StatusMethodNotAllowed,
			mapper.ToErrorResponse(fmt.Errorf("method not allowed")))
		return
...
func (h *GameHandler) GetHistoryByToken(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
		mapper.WriteJSON(w, http.StatusMethodNotAllowed,
			mapper.ToErrorResponse(fmt.Errorf("method not allowed")))
		return
	...
	games, err := h.gameService.GetFinishedGamesByID(r.Context(), userID)
	
	if err != nil {
		mapper.WriteJSON(w, http.StatusBadRequest, mapper.ToErrorResponse(fmt.Errorf("cant get finished games")))
		return
	}
...
func (h *GameHandler) GetLeaderboard(w http.ResponseWriter, r *http.Request) {
    ...
    response := make([]map[string]interface{}, 0, len(players))
    for _, leader := range players {
